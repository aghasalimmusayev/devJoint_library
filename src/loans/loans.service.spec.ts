...
// create()/returnBook() are wrapped in @Transactional(). It needs its
// async-local-storage context initialized, and a registered DataSource whose
// .transaction() it can call. There's no real DB here, so a stub that just
// invokes the callback directly is enough — the actual save() calls still go
// through the mocked repositories below, untouched by this stub.
initializeTransactionalContext();
addTransactionalDataSource({
    name: 'default',
    dataSource: { transaction: (cb: (manager: unknown) => unknown) => cb({}) } as any,
    patch: false,
});
...
describe('LoansService', () => {
    ...
    describe('create', () => {
        ...
        // @Transactional() only rolls back the real DB transaction if the error
        // actually propagates out of the method. This proves it isn't swallowed
        // when the second write (the loan) fails after the first (the book) succeeded —
        // that's the one thing a unit test with mocked repositories can verify;
        // that the DB rollback itself happens is exercised against a real DataSource.
        it('should propagate the error and not swallow it when the loan write fails after the book was already decremented', async () => {
            const book = { id: 'book-1', availableCopies: 2 };
            bookRepository.findOne!.mockResolvedValue(book);
            bookRepository.save!.mockResolvedValue({ ...book, availableCopies: 1 });
            loanRepository.create!.mockReturnValue({ ...dto, userId: currentUserId });
            loanRepository.save!.mockRejectedValue(new Error('DB connection lost'));
            await expect(service.create(dto, currentUserId)).rejects.toThrow(
                'DB connection lost',
            );
        });
    ...
    describe('returnBook', () => {
        ...
        it('should propagate the error and not swallow it when the book write fails after the loan was already marked returned', async () => {
            const loan = { id: 'loan-1', bookId: 'book-1', returnedAt: null };
            const book = { id: 'book-1', availableCopies: 1 };
            loanRepository.findOne!.mockResolvedValue(loan);
            loanRepository.save!.mockResolvedValue({ ...loan, returnedAt: new Date() });
            bookRepository.findOne!.mockResolvedValue(book);
            bookRepository.save!.mockRejectedValue(new Error('DB connection lost'));
            await expect(service.returnBook('loan-1')).rejects.toThrow(
                'DB connection lost',
            );
        });
